layui.use(['form','layer','jquery','laypage','table','laytpl'],function(){
    ...
    table.render({
        elem: '#test'
        ,url:'/manage/course_contorller/getCourse?state=1'
        ,cellMinWidth:80
        ,cols: [[
    ...
    table.on('tool(demo)', function(obj){
        ...
        if(obj.event === 'shownews'){
            ...
            layer.confirm('真的拒绝通过么', function(index){
                $.post("/manage/course_contorller/saveCourseStatue",{id:data.id,val:3},function(data){
                    if(data.error_code==0){
            ...
            layer.confirm('真的审核通过么', function(index){
                $.post("/manage/course_contorller/saveCourseStatue",{id:data.id,val:2},function(data){
                    if(data.error_code==0){
                        layer.msg("成功",{time:400},function(){
                            obj.state();
                            layer.close(index);
