...
var openid=$('#openid').val();
    table.render({
        elem: '#test'
        ,url:'/manage/user_contorller/report_list?openid='+openid
        ,cols: [[
            {type:'checkbox'},
            {field:'id', title: 'ID', sort: true}
            ,{field:'title',title: '报表名称'}
            ,{field:'rtime',  title: '报表生成时间'}
    ...
    table.on('tool(demo)', function(obj){
        ...
        if(obj.event === 'del'){
            layer.confirm('真的删除行么', function(index){
                $.post("/manage/user_contorller/delreport",{id:data.id},function(data){
                    if(data.error_code==0){
            ...
            layer.open({
                ...
                area: ['100%','100%'], //宽高
                content: "/manage/user_contorller/showreport?id="+data.id
            });
            ...
            layer.open({
                ...
                skin: 'layui-layer-molv', //加上边框
                area: ['100%','100%'], //宽高
                content: "/manage/user_contorller/editreport?id="+data.id+"&openid="+openid
            });
        }
    });
