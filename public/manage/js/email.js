...
var openid=$('#openid').val();
    table.render({
        elem: '#test'
        ,url:'/manage/user_contorller/email_list?id='+id+'&type='+type+'&openid='+openid
        ,cols: [[
            {type:'checkbox'},
    ...
    table.on('tool(demo)', function(obj){
        ...
        if(obj.event === 'detail'){
            layer.open({
                ...
                area: ['100%','100%'], //宽高
                content: "/manage/user_contorller/showemail?id="+data.id
            });
            //layer.msg('ID：'+ data.uid + ' 的查看操作');
        } else if(obj.event === 'del'){
            layer.confirm('真的删除行么', function(index){
                $.post("/manage/user_contorller/delemail",{id:data.id},function(data){
                    if(data.error_code==0){
            ...
            layer.open({
                ...
                skin: 'layui-layer-molv', //加上边框
                area: ['100%','100%'], //宽高
                content: "/manage/user_contorller/fan_email?id="+data.id+"&save=save&openid="+data.openid+"&btime="+data.btime
            });
        }else if(obj.event === 'xunlian'){//查看与 编辑用户训练计划
            layer.open({
                type:2,
                skin: 'layui-layer-molv', //加上边框
                area: ['100%','100%'], //宽高
                content: "/manage/user_contorller/xunlian?id="+data.id+"&openid="+data.openid+"&btime="+data.btime
            });
        }else if(obj.event === 'addxunlian'){//添加 用户 训练计划
            layer.open({
                type:2,
                skin: 'layui-layer-molv', //加上边框
                area: ['100%','100%'], //宽高
                content: "/manage/user_contorller/user_xun?id="+data.id+"&openid="+data.openid+"&btime="+data.btime
            });
        }else if(obj.event === 'yulans'){
            layer.open({
                type:2,
                skin: 'layui-layer-molv', //加上边框
                area: ['100%','100%'], //宽高
                content: "/manage/user_contorller/yulan?id="+data.id+"&btime="+data.btime
            });
        }else if(obj.event === 'edityulan'){
            layer.open({
                type:2,
                skin: 'layui-layer-molv', //加上边框
                area: ['100%','100%'], //宽高
                content: "/manage/user_contorller/edityulan?id="+data.id+"&btime="+data.btime
            });
        }
    });
