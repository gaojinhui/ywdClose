layui.use(['form',"table","upload","laytpl"], function(){
    ...
    upload.render({
        ...
        ,done: function(res){
            if(res.error_code==0){
                layer.msg('上传成功',{time:800},function(){
                    $(".site-demo-up img").attr("src",res.data);
                    $(".site-demo-up img").show();
...
//数字前置补零
    ...
    table.on('edit(demo)', function(obj){
        ...
        $.post("/manage/user_contorller/saveInfo",{id:data.id,field:field,val:value},function(res){
    ...
    table.render({
        elem: '#test'
        ,url:'/manage/user_contorller/getAdminUser'
        ,cellMinWidth:80
        ,cols: [[
            {type:'checkbox'},
    ...
    // });
form.on('switch(upCourse)', function(obj){
        var status=this.value;
        var bid = $(this).attr("bid");
        if(status==1){status=2}else{status=1}
        console.log(status);
        $.post("/manage/user_contorller/edit_user",{id:bid,val:status},function(data){
            if(data.error_code==0){
                layer.msg("修改成功");
            }else{
                layer.msg("修改失败");
    ...
    table.on('tool(demo)', function(obj){
        ...
        if(obj.event === 'detail'){
            layer.open({
                ...
                area: ['100%','100%'], //宽高
                content: "/manage/user_contorller/show_userinfo?id="+data.id
            });
            ...
            layer.open({
                ...
                skin: 'layui-layer-molv', //加上边框
                area: ['100%','100%'], //宽高
                content: "/manage/user_contorller/email?id="+data.id+"&save=save"
            });
        }
    });
