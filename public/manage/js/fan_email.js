...
layui.use(['form',"table","upload","laytpl"], function(){
        ...
        ,done: function(res){
            if(res.error_code==0){
                layer.msg('上传成功',{time:800},function(){
                    $("#litpica1").attr("src",res.data);
                    $("#litpica1").show();
                    document.getElementById('litpic1').value=res.data;
                });
            }else{
    ...
    upload.render({
        ...
        ,done: function(res){
            if(res.error_code==0){
                layer.msg('上传成功',{time:800},function(){
                    $("#litpicb200").attr("src",res.data);
                    $("#litpicb200").show();
                    document.getElementById('litpic200').value=res.data;
                });
            }else{
                layer.msg('上传失败',{time:800});
            }
    ...
    form.on("submit(addNews)",function(data){
        ...
        $.post("/manage/user_contorller/email_fan",data.field,function(data){
           // layer.close(index);
