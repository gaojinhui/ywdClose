...
//数字前置补零
    ...
    table.on('edit(demo)', function(obj){
        ...
        $.post("/manage/index/update_paertner_handle",{id:data.id,field:field,val:value},function(res){
    ...
    table.render({
        elem: '#test'
        ,url:'/manage/index/partnerlist'
        ,cellMinWidth:80
        ,cols: [[
            {type:'checkbox'},
            {field:'id', width:200, title: 'ID', sort: true}
            ,{field:'conpanyname', title: '标题', edit: 'text'}
    ...
    form.on('switch(sexDemo)', function(obj){
        ...
        console.log(status);
        $.post("/manage/index/update_parener_status",{id:bid,val:status},function(data){
    ...
    table.on('tool(demo)', function(obj){
        ...
        if(obj.event === 'detail'){
            ...
            layer.confirm('真的删除行么', function(index){
                $.post("/manage/index/delpaertner",{id:data.id},function(data){
                    if(data.error_code==0){
            ...
            layer.open({
                ...
                area: ['100%','100%'], //宽高
                content: "/manage/index/update_partner?id="+data.id,
                end:function(){
    ...
    var $ = layui.$, active = {
        ...
        ,addData: function(){
            layer.open({
                ...
                skin: 'layui-layer-molv', //加上边框
                area: ['100%','100%'], //宽高
                content: "/manage/index/showaddpartner",
                end:function(){
                    window.location.reload();
                }
            });
